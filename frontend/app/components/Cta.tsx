...
export default function CTA({block}: CtaProps) {
  ...
  return (
    <section className={isDark ? 'relative dark dark:bg-dark' : 'relative'}>
      <div className="absolute inset-0 bg-light/50" />
      <div className="container relative">
        <div className="grid lg:grid-cols-2 gap-12 py-12">
          ...
          >
            {eyebrow && (
              <span className="text-sm uppercase dark:text-white tracking-wide opacity-70">
                {eyebrow}
              </span>
            )}
            {heading && (
            ...
            {button?.buttonText && button?.link && (
              <div className="flex mt-4">
                <ResolvedLink
                  link={button?.link}
                  className="rounded-full flex gap-2 text-sm font-medium whitespace-nowrap items-center bg-blue hover:bg-blue/90 py-3 px-6 text-white transition-colors duration-200"
                >
                  {button?.buttonText}
                </ResolvedLink>
              </div>
